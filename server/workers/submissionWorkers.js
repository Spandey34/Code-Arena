...
// Adjust the path to point back to your root directory where .env sits
...
// 🚨 CRITICAL: Catch errors on the publisher client to prevent unhandled ECONNREFUSED crashes!
redisPublisher.on("error", (err) => {
  console.error("Redis Publisher Error:", err.message);
});

redisPublisher.on("connect", () => {
  console.log("Redis Publisher client successfully connected.");
});
  ...
  async (job) => {
    ...
    try {
      ...
      if (execResult.status === 'success') {
         let  allPassed = details.testResults.every(t => t.passed === true);
